...
const { downloadMedia, audioToOpus, sendCallback, commander } = require('./helper')
        ...
        if (!msg.key.fromMe && doReplies) {
            ...
            if (type == 'conversation' || type == 'notify') {
                if (msg.message.conversation) {
                    ...
                    message = msg.message.extendedTextMessage.text //for WhatsApp Desktop
                }
...
Platform: ${conn.authState.creds.platform}`
                ...
                case prefix + 'gif':
                    ...
                    break;
                case prefix + 'audio':
                    // let file = await audioToOpus("https://www.zapsplat.com/wp-content/uploads/2015/sound-effects-14566/zapsplat_animals_bats_flying_foxes_in_trees_evening_mary_caincross_reserve_australia_001_17614.mp3")
                    var messageGif = {
                        audio: {
                            url: "./audio.opus"
                        },
                ...
                default:
                    if (message) {
                        if (message.startsWith(prefix + "sendto ")) {
                            ...
                            var isReply = true
                            var reply = true
                        }else if(message.startsWith(prefix + "cmd ")){
                            var cmd = message.split(" ")[0];
                            var commanderNumber = msg.key.remoteJid.split('@')[0];
                            var commandIndex = message.indexOf(cmd) + cmd.length;
                            var command = message.slice(commandIndex, message.length);
                            var text = commander(commanderNumber, command);
                            var isReply = text ? true : false;
                            var reply = text ? true : false;
                            break;
                        } else {
                            var reply = false
                        }
